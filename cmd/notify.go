...
func notifyNewRelease(w io.Writer) error {
	...
	if c != nil && !c.Expired() {
		...
		r, _, err := cl.Repositories.GetLatestRelease(context.Background(), "koki-develop", "todoist-cli")
		if err != nil {
			return err
		}
		release = *r
	...
	if util.Version(*release.Name).Newer(util.Version(version)) {
		...
		link := fmt.Sprintf("See: %s", styles.StyleLink.Render(fmt.Sprintf("https://github.com/koki-develop/todoist-cli/releases/%s", *release.Name)))
